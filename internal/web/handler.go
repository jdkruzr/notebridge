...
// TaskStoreInterface defines the interface for task operations.
...
// SyncNotifier interface for task sync notifications.
...
func (h *Handler) handleFiles(w http.ResponseWriter, r *http.Request) {
	if h.noteStore == nil {
		...
		if err := h.tmpl.ExecuteTemplate(w, "index.html", map[string]interface{}{
			"filesError": "NB_STORAGE_PATH is not configured",
			"activeTab":  "files",
		}); err != nil {
			h.logger.Error("failed to render template", "error", err)
		}
