...
type Pipeline struct {
	...
	eventBus  *events.EventBus // EventBus for primary file detection
	logger    *slog.Logger
...
type Config struct {
	...
	EventBus  *events.EventBus // EventBus for subscribing to FileUploaded
	Logger    *slog.Logger
}
...
func (p *Pipeline) runAll(ctx context.Context) {
	...
	// Subscribe to FileUploaded events as primary detection
	if p.eventBus != nil {
...
// (NoteBridge's own RECOGNTEXT injection), the file is skipped. If they differ (user edit
...
func (p *Pipeline) enqueue(ctx context.Context, path string) {
	...
	// UpsertFile is defined in NoteStore for exactly this purpose.
	...
	// Compare stored hash with current file to distinguish NoteBridge's own write from a user edit.
	...
	if err == nil && job != nil && job.Status == processor.StatusDone {
		...
		if storedHash != "" && storedHash == currentHash {
			return // hashes match — NoteBridge wrote this file, no re-processing needed
		}
