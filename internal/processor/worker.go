...
func (s *Store) executeJob(ctx context.Context, job *Job) error {
	...
	// Extract TITLE and KEYWORD block text from the note footer.
	...
	if _, err := s.db.ExecContext(ctx,
		"UPDATE jobs SET ocr_source=? WHERE id=?",
		"api", job.ID,
	); err != nil {
...
func (s *Store) ensureBackup(ctx context.Context, path string) error {
	...
	if _, err := s.db.ExecContext(ctx,
		"UPDATE notes SET backup_path=? WHERE path=?",
		dst, path,
	); err != nil {
...
// extractNoteTitle reads TITLE block labels from the note footer.
...
// extractNoteKeywords reads KEYWORD block text from the note footer.
