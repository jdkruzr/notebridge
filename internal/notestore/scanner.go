		...
		if err != nil {
			...
			if errors.Is(err, sql.ErrNoRows) {
				...
				_, insertErr := s.db.ExecContext(ctx, `
					INSERT INTO notes (path, rel_path, file_type, size_bytes, mtime)
					VALUES (?, ?, ?, ?, ?)`,
					path, relPath, string(ft), sizeBytes, mtimeUnix)
				if insertErr != nil {
			...
			_, updateErr := s.db.ExecContext(ctx, `
				UPDATE notes SET size_bytes=?, mtime=? WHERE path=?`,
				sizeBytes, mtimeUnix, path)
			if updateErr != nil {
...
func (s *Store) UpsertFile(ctx context.Context, path string) error {
	...
	_, err = s.db.ExecContext(ctx, `
		...
		path, relPath, string(ft), info.Size(), info.ModTime().Unix(),
	)
	return err
}
